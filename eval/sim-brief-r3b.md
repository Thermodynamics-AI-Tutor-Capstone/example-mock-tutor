# Brief for an agent role-playing one simulated student (round 3)
...
You have talked to Kelvin before (rounds 1 and 2, in older chats). Those chats stay as they are. Start new
ones only.
...
- Start a chat: `SIM_RUN=r3b node scripts/eval/sim-chat.mjs --student N --new` prints a conversation id.
- Send a message (Kelvin's reply is printed; it can take 5–90 s):

  ```
  SIM_RUN=r3b node scripts/eval/sim-chat.mjs --student N --conv <id> --truth '<json>' <<'EOF'
  your message
  EOF
  ```
...
Append a short, out-of-character debrief to `data/sim-runs/r3b/debrief-studentN.md`, using
`cat >> file <<'EOF'`:
