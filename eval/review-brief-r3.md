...
- `data/sim-runs/r3/transcript-studentN.md` for each of YOUR students (listed in your prompt). Each
...
- The persona for student N: `node scripts/eval/sim-persona.mjs N eval/students-r2.yml` (the hidden misconceptions are the
  answer key).
- The student's own debrief, `data/sim-runs/r3/debrief-studentN.md`. It is a second opinion only.
...
Write everything to `data/sim-runs/r3/review-<your label>.md`, using `cat > file <<'EOF'`.
