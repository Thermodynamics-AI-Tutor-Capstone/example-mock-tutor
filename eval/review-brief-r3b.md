# Brief for an agent reviewing the simulated-student chats (round 3)
...
- `data/sim-runs/r3b/transcript-studentN.md` for each of YOUR students (listed in your prompt). Each
...
- The persona for student N: `node scripts/eval/sim-persona.mjs N eval/students-r2.yml` (the hidden misconceptions are the
  answer key).
- The student's own debrief, `data/sim-runs/r3b/debrief-studentN.md`. It is a second opinion only.
...
```
node --input-type=module -e "import {state} from '/Users/lancestreuber/Desktop/Capstone/example-mock-tutor/.claude/worktrees/evals/lib/properties.js'; console.log(JSON.stringify(state('water',{P:200,T:300})))"
```
...
- Found what needed correcting: <yes / partly / no>. For a student with no misconception (students 2 and 7), did Kelvin avoid inventing problems and still add value?
- Thoroughness: <1–5> — <was the correction complete (the belief, not just the number), and was the student left able to do the next problem>
...
After all your chats, add a short `## Summary` with totals: misconceptions shown vs addressed, false corrections,
accuracy errors (count and list), early give-aways, the mean thoroughness and helpfulness scores, and the 2–3 most
important patterns, good or bad.

Write everything to `data/sim-runs/r3b/review-<your label>.md`, using `cat > file <<'EOF'`.
