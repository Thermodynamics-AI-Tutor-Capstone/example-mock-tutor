...
- Start a chat: `SIM_RUN=r3 node scripts/eval/sim-chat.mjs --student N --new` prints a conversation id.
- Send a message (Kelvin's reply is printed; it can take 5–90 s):

  ```
  SIM_RUN=r3 node scripts/eval/sim-chat.mjs --student N --conv <id> --truth '<json>' <<'EOF'
  your message
  EOF
  ```
...
Append a short, out-of-character debrief to `data/sim-runs/r3/debrief-studentN.md`, using
`cat >> file <<'EOF'`:
