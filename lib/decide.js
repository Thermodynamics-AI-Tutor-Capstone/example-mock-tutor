...
export function normaliseRead(result, misconceptions) {
...
// Never throws: when Jev is unavailable the turn goes ahead on the keyword read above and the
// policy stays at its most conservative for everything the keywords can't see.
export async function readTurn({ history, attachmentsText, tutoringState, styles, knowledge, currentStyleId, signal, enabled = true }) {
  if (!enabled) return heuristicRead({ history, attachmentsText }, 'Jev is turned off in settings');
  const config = deciderConfig();
  const problem = deciderProblem(config);
  if (problem) return heuristicRead({ history, attachmentsText }, problem);
  ...
  try {
    const result = await systemOne({ state, questions: buildQuestions({ styles, misconceptions }), signal, config });
    return normaliseRead(result, misconceptions);
  } catch (e) {
    if (signal?.aborted) throw e;
    console.warn(`Decider unavailable this turn: ${e.message}`);
    return heuristicRead({ history, attachmentsText }, e.message);
  }
}
