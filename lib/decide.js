...
export async function readTurn({ history, attachmentsText, tutoringState, styles, knowledge, currentStyleId, signal, enabled = true }) {
  ...
  if (enabled) {
    ...
    if (problem) why.push(`Jev: ${problem}`);
      ...
      try {
        return normaliseRead(await systemOne({ state, questions, signal, config }), misconceptions, 'jev');
      } catch (e) {
  ...
  if (!problem) {
    try {
      const read = normaliseRead(await backupSystemOne({ state, questions, signal, config: backup }), misconceptions, 'llm');
