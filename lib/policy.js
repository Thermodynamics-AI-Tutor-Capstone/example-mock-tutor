...
import { isModelRead } from './decide.js';
...
export function routeStyle({ pinnedId, styles, read, state, usable = () => true, defaultId, thresholds = loadPolicy().thresholds }) {
  ...
  if (!pick) {
    ...
    if (current) return { style: current, auto: true, reason: isModelRead(read) ? 'kept (no style answer)' : 'kept (no classifier read)' };
...
export function applyPolicy({ style, state: prior = {}, read, cards = [], policy = loadPolicy(), now = new Date() }) {
  ...
  // stop it.
  const stalled = isModelRead(read) && !counted && STALL_INTENTS.has(intent) && !(read.showsWork >= 0.3);
  ...
  const turn = {
    ...
    readAvailable: isModelRead(read),
    readKind: read?.provider || 'none',
    readReason: isModelRead(read) ? null : read?.reason || null,
    events,
  };
  return { state, turn };
}
