...
async function deleteOriginal(row) {
  try {
    if (row.storage === 'blob') {
      const { del } = await import('@vercel/blob');
      await del(row.storage_key);
    } else {
      await fsp.rm(path.join(LOCAL_DIR, row.storage_key), { force: true });
    }
  } catch (e) {
    console.error('Could not delete original upload:', e.message);
  }
}
...
export async function getAttachmentRow(id, userId) {
  const { rows } = await query(`SELECT ${COLS} FROM attachments WHERE id = $1 AND user_id = $2`, [id, userId]);
  return rows[0] || null;
}

export async function listAttachments(conversationId, userId) {
  const { rows } = await query(
    `SELECT ${COLS} FROM attachments WHERE conversation_id = $1 AND user_id = $2 ORDER BY created_at`,
...
export async function createAttachment({ user, conversationId, filename, buffer }) {
  ...
  const storage = await storeOriginal(storageKey, buffer, mime);

  let markdown = '';
  let source = kind === 'vision' ? 'vision' : 'extracted';
  let status = kind === 'vision' ? 'needs_review' : 'ready';
  try {
    if (ext === 'pdf') {
      ...
      markdown = kind === 'vision' ? (await imageToMarkdown(ext, buffer)).markdown : await documentToMarkdown(ext, buffer);
    }
    if (!markdown.trim()) throw httpError(422, 'No text could be read from this file.');
  } catch (e) {
    await deleteOriginal({ storage, storage_key: storageKey });
  ...
  const { rows } = await query(
    ...
    [id, user.id, conversationId, name, mime, buffer.length, kind, source, status, markdown, storage, storageKey]
  );
  return rows[0];
}
...
export async function deleteAttachment(id, user) {
  const row = await getAttachmentRow(id, user.id);
  if (!row) return false;
  await query('DELETE FROM attachments WHERE id = $1 AND user_id = $2', [id, user.id]);
  await deleteOriginal(row);
  return true;
}
