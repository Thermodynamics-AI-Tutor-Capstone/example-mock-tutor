...
export async function createAttachment({ user, conversationId, filename, buffer }) {
  ...
  try {
    ...
    // An upload that can't be read is removed entirely: it never becomes visible to the tutor.
    await query('DELETE FROM attachments WHERE id = $1', [id]).catch(() => {});
...
export async function processingCount(conversationId, userId) {
  const { rows } = await query(
    `SELECT count(*)::int AS n FROM attachments
     WHERE conversation_id = $1 AND user_id = $2 AND status = 'processing'
