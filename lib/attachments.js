...
async function pdfToMarkdown(buffer, tag = {}) {
  ...
  try {
    ...
    async function worker() {
      while (next < toRead.length) {
        ...
        page.markdown = (await imageToMarkdown('png', Buffer.from(png), tag)).markdown;
        page.vision = true;
      }
    }
...
async function imageToMarkdown(ext, buffer, tag = {}) {
  ...
  await recordUsage(
    deepseekUsageRow({ model: conn.model, usage: data?.usage, purpose: 'attachment_vision', conversationId: tag.conversationId, userId: tag.userId, latencyMs: Date.now() - started, ok: r.ok, error: r.ok ? null : `HTTP ${r.status}` })
  );
...
export async function createAttachment({ user, conversationId, filename, buffer }) {
  ...
  try {
    ...
    if (ext === 'pdf') {
      const result = await pdfToMarkdown(buffer, { conversationId, userId: user.id });
      ...
      markdown = useVision ? (await imageToMarkdown(ext, buffer, { conversationId, userId: user.id })).markdown : await documentToMarkdown(ext, buffer);
    }
