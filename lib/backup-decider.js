...
export async function backupSystemOne({ state, questions, signal, config = backupConfig(), retries = 1, usage = {} }) {
  const started = Date.now();
  const tag = { model: config.model, purpose: usage.purpose || 'backup', conversationId: usage.conversationId, userId: usage.userId };
  try {
    const result = await backupCall({ state, questions, signal, config, retries });
    await recordUsage(deepseekUsageRow({ ...tag, usage: result.usage, latencyMs: result.latencyMs }));
    return { ...result, costUsd: result.usage ? deepseekUsageRow({ ...tag, usage: result.usage }).costUsd : null };
  } catch (e) {
    await recordUsage(deepseekUsageRow({ ...tag, usage: null, latencyMs: Date.now() - started, ok: false, error: e.message }));
    throw e;
  }
}

async function backupCall({ state, questions, signal, config, retries }) {
