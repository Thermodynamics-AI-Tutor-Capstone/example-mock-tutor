...
// build/knowledge-index.json. Reading agent/kb/** off disk is the build script's job
...
// Mirrors enums.status in agent/kb/taxonomy.yml, plus `stub` for cards this compile synthesises.
...
/**
 * agent/knowledge path → the slug used by agent/kb/context/<slug>.md and agent/kb/sources/<slug>.md.
...
function toSources(value) {
  ...
  for (const entry of list) {
    ...
    // A card may cite a file in agent/knowledge/, an external document (the published sample
...
/**
 * Parse one agent/kb/**.md card file.
...
/**
 * Parse the hand-authored agent/kb/taxonomy.yml. Deliberately tolerant about shape (a human writes
...
export function parseTaxonomy(text) {
  ...
  // (which is what agent/kb/taxonomy.yml does: one row per lecture). Both are accepted.
...
const STUB_BODY =
  ...
  'This card was synthesised at build time from `agent/kb/taxonomy.yml`; nothing has been read from ' +
...
/** Make agent/kb/symbols.md openable by the model and browsable by students. */
...
export function renderL0({
  ...
  const trimProse = () => {
    ...
    const pointer = 'The rest of the hand-written preamble is in `agent/kb/INDEX.md`.';
