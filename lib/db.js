...
-- Admins see /admin. Set only in the database, when Lance says so; nothing in the app can set it.
ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS is_admin boolean;
...
-- Solve first (lib/solver.js): the server's own tool-checked solution to the problem in progress.
-- No foreign key, so nothing removes it with a chat.
...
CREATE INDEX IF NOT EXISTS reference_solutions_conversation_idx ON reference_solutions (conversation_id, created_at DESC);
