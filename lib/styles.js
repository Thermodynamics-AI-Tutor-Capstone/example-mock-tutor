...
export const DEFAULT_STYLE_ID = 'office-hours';
export const AUTO_STYLE = Object.freeze({
  id: 'auto',
  name: 'Auto',
  description: 'Kelvin picks how to teach from what you ask.',
  icon: '✨',
  order: 0,
});
...
// route_when is what picks this style in Auto: Jev reads use_for, not_for and the examples ("Jev
// picks"), and the tutoring model reads use_for, not_for and leave_when from its style index ("skills
// pick"). intents are the fallback when there is no model answer at all.
function readRouteWhen(raw) {
  const r = raw && typeof raw === 'object' ? raw : {};
  const text = (v) => (typeof v === 'string' ? v.replace(/\s+/g, ' ').trim() : '');
  return { use_for: text(r.use_for), not_for: text(r.not_for), leave_when: text(r.leave_when), examples: asStrings(r.examples), intents: asStrings(r.intents) };
}
...
function readStyle(dir) {
  ...
  return {
    ...
    routeWhen: readRouteWhen(raw.route_when),
    mergedInto: typeof raw.merged_into === 'string' && raw.merged_into.trim() ? raw.merged_into.trim() : null,
...
export function listStyles(check) {
  const auto = { ...AUTO_STYLE, ui: { figures: false, mermaid: false }, available: true, default: true, auto: true };
...
// A style that was merged into another (merged_into) answers to its old id, so chats, pins and
// settings saved before the merge land on the style that replaced it.
export function resolveStyleId(id) {
  if (!id || id === AUTO_STYLE.id) return id;
  let s = findStyle(id);
  for (let hops = 0; s && !s.enabled && s.mergedInto && hops < 3; hops++) s = findStyle(s.mergedInto);
  return s ? s.id : id;
}

export function isStyleId(id) {
  return id === AUTO_STYLE.id || Boolean(findStyle(id));
}
