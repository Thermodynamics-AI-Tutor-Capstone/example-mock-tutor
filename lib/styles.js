...
// route_when is what the Auto router (Jev) reads to decide when this style fits: use_for, not_for,
// a few example student messages, and the intents it serves when Jev has no style answer.
function readRouteWhen(raw) {
  const r = raw && typeof raw === 'object' ? raw : {};
  const text = (v) => (typeof v === 'string' ? v.replace(/\s+/g, ' ').trim() : '');
  return { use_for: text(r.use_for), not_for: text(r.not_for), examples: asStrings(r.examples), intents: asStrings(r.intents) };
}
