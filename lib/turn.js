...
export async function auditReply({ history, reply, state, signal, useJev = true }) {
  ...
  try {
    const result = viaJev
      ? await systemOne({ state: auditState, questions: AUDIT_QUESTIONS, signal, config: jev }).catch((e) => {
          if (signal?.aborted || backupProblem(backup)) throw e;
          return backupSystemOne({ state: auditState, questions: AUDIT_QUESTIONS, signal, config: backup });
        })
      : await backupSystemOne({ state: auditState, questions: AUDIT_QUESTIONS, signal, config: backup });
...
export async function finishTurn({ prepared, conversationId, userId, messageId = null, history, reply, agent = null, signal, audit: runAudit = true }) {
  ...
  const [audit] = await Promise.all([
    runAudit ? auditReply({ history, reply, state: agent?.state || state, signal, useJev: prepared.useJev !== false }) : null,
