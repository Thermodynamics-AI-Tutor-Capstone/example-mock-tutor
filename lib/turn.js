...
import { loadStyles, DEFAULT_STYLE_ID, resolveStyleId } from './styles.js';
...
import { loadReference, referenceSection } from './solver.js';
...
export async function prepareTurn({ conversationId, userId, pinnedStyleId, history, attachmentsText = '', usable = () => true, signal, useJev = true, styleRouter = 'jev' }) {
  ...
  pinnedStyleId = resolveStyleId(pinnedStyleId);
  ...
  if (priorState.routedStyle) priorState.routedStyle = resolveStyleId(priorState.routedStyle);
  ...
  // Solve first: the server's reference solution for the problem in progress, when there is one.
  const reference = conversationId ? await loadReference(conversationId, state.problemStartedAt || null).catch(() => null) : null;
  const extraSections = [studentModelSection(studentModel)];
  if (reference?.ok) extraSections.push(referenceSection(reference));
  if (style?.id === 'probe') extraSections.push(practiceSection(studentModel));
