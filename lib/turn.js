...
import { readTurn, misconceptionCards } from './decide.js';
...
// eval's "without Jev" arm). Everything Jev would decide falls back to what code can do alone.
export async function prepareTurn({ conversationId, userId, pinnedStyleId, history, attachmentsText = '', usable = () => true, signal, useJev = true }) {
...
export function decisionSummary({ read, route, style, turn }) {
...
export function evidenceFromRead(read, history, policy = loadPolicy()) {
  if (!read || read.provider !== 'jev') return [];
...
export async function auditReply({ history, reply, state, signal }) {
  const config = deciderConfig();
  if (deciderProblem(config) || !reply || !reply.trim()) return null;
  try {
...
// audit defaults to following the Jev switch: a student who turned Jev off gets no Jev calls at all,
// not even the after-the-fact audit. The eval passes audit: true so both arms are measured alike.
export async function finishTurn({ prepared, conversationId, userId, messageId = null, history, reply, agent = null, signal, audit: runAudit = prepared.useJev !== false }) {
  const { read, state, turn, route, style } = prepared;
  const evidence = evidenceFromRead(read, history);
  const [audit] = await Promise.all([
    runAudit ? auditReply({ history, reply, state: agent?.state || state, signal }) : null,
      ...
      await query(
        ...
        [
          ...
          JSON.stringify({
            ...
            summary: decisionSummary(prepared),
          }),
