...
// a pure fold over that ledger and keeps only what is currently believed. Nothing is ever deleted: a
// chat the student deletes is hidden, and its evidence stops counting (loadEvidence skips it).
//
// Evidence comes from four places: Jev's per-turn read (misconception signals), the tutoring
// model's tool calls (confirmed / repaired misconceptions, hypotheses, practice results), the
// server, and the student, who can see and correct all of it in Settings (source "student": a
// dismissed misconception, an edited or removed note, a reset practice record, a note of their own).
// Every belief expires: after staleDays without new evidence it is shown as "(old)".
...
export const EVIDENCE_KINDS = [
  ...
  'hypothesis_retired',
  'practice_result',
  'misconception_dismissed',
  'practice_reset',
];
export const EVIDENCE_SOURCES = ['jev', 'tutor', 'server', 'student'];
...
export function foldStudentModel(rows, { now = new Date(), config = DEFAULTS } = {}) {
  ...
  for (const row of rows || []) {
    ...
    switch (row.kind) {
      ...
      // The student says it isn't true: it drops out, and only new evidence brings it back.
      case 'misconception_dismissed':
        miscs.delete(row.ref);
        break;
      case 'hypothesis': {
        const prev = hyps.get(row.ref);
        hyps.set(row.ref, {
          ref: row.ref,
          source: row.source || 'tutor',
...
export function studentModelSection(model, { maxLines = 14 } = {}) {
  ...
  for (const h of [...(m.hypotheses || [])].sort((a, b) => String(b.updatedAt).localeCompare(String(a.updatedAt)))) {
    ...
    const label = h.source === 'student' ? 'The student says (set in Settings)' : 'Hypothesis';
    lines.push(`- **${label}:** about \`${h.about}\` — ${h.belief}${h.note ? `: ${h.note}` : ''} (${conf}${day(h.updatedAt)})${old(h)}`);
  }
