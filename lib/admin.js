...
// filter splits students into real, test (the simulated-student accounts) and eval (the eval account
// and synthetic eval users). Nothing here writes.

const APP_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const EVAL_RESULTS_DIR = path.join(APP_DIR, 'eval', 'results');
export const GROUPS = ['all', 'real', 'test', 'eval'];
...
export function categoryOf({ user_id: userId, email }) {
  ...
  if (/^test\d+@tutor\.(test|com)$/.test(e)) return 'test';
  if (e.endsWith('@thermo-tutor.test') || String(userId).startsWith('eval-')) return 'eval';
  return 'real';
}
...
export async function overview(group) {
  ...
  const [chats, msgs, days, styles, cost, active] = await Promise.all([
    ...
    query(
      `SELECT to_char(date_trunc('day', m.created_at), 'YYYY-MM-DD') AS day, c.user_id, count(*) AS n
         FROM messages m JOIN conversations c ON c.id = m.conversation_id
        WHERE c.user_id = ANY($1::text[]) AND m.role = 'user' AND m.created_at > now() - interval '30 days'
  ...
  const byCategory = { real: 0, test: 0, eval: 0 };
  for (const s of members) byCategory[s.category] += 1;
  const perDay = {};
  for (const r of days.rows) {
    const cat = byId.get(r.user_id)?.category || 'real';
    perDay[r.day] ||= { real: 0, test: 0, eval: 0 };
    perDay[r.day][cat] += num(r.n);
  }
  ...
  return {
    ...
    messagesPerDay: Object.entries(perDay).map(([day, v]) => ({ day, ...v })),
...
// ── Misconceptions ──────────────────────────────────────────────────────────────────────────────
export async function misconceptions(group) {
  ...
  const { rows } = await query(
    `SELECT ref,
      ...
      WHERE user_id = ANY($1::text[]) AND ref LIKE 'misc:%'
