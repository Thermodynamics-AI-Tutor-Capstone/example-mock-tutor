...
export async function getProfile(userId) {
  ...
  const { rows } = await query(`SELECT ${[...PROFILE_FIELDS, ...switches].join(', ')}, onboarded_at FROM user_profiles WHERE user_id = $1`, [userId]);
  const row = rows[0];
  if (!row) return null;
  const out = {};
...
export async function saveProfile(user, body, { validStyle } = {}) {
  ...
  const cols = [...PROFILE_FIELDS, ...Object.keys(PROFILE_SWITCHES)];
  await query(
