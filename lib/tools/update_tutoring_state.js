...
export default async function run(args, ctx) {
  ...
  if (typeof args.problem_status === 'string' && PROBLEM_STATUSES.includes(args.problem_status)) {
    // A problem the student hasn't carried to their own final answer can't be finished (the eval
    // caught "Done — marked this one finished" on a student who then repeated the same slip).
    if (args.problem_status === 'finished' && ctx.turn && !ctx.turn.completeAttempt) {
      notes.push('not marked finished: the student has not made a complete attempt at this problem yet');
    } else {
