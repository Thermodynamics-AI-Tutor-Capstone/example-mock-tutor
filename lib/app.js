...
async function getConversationRow(id) {
  const { rows } = await query('SELECT id, title, style, created_at, updated_at FROM conversations WHERE id = $1', [id]);
  return rows[0] || null;
}

async function listConversations() {
  const { rows } = await query(
    'SELECT id, title, style, created_at, updated_at FROM conversations ORDER BY updated_at DESC, created_at DESC'
  );
  return rows.map(summary);
}

async function createConversation(style = null) {
  const { rows } = await query(
    'INSERT INTO conversations (id, title, style) VALUES ($1, $2, $3) RETURNING id, title, style, created_at, updated_at',
    [crypto.randomUUID(), NEW_TITLE, style]
  );
...
async function route(req, res, pathname) {
  ...
  if (pathname === '/api/conversations') {
    ...
    if (method === 'POST') {
      const body = await readJsonBody(req);
      return sendJson(res, 201, await createConversation(validStyleId(body.style)));
    }
    ...
    if (method === 'PATCH') {
      ...
      const { rows } = await query(
        `UPDATE conversations SET
           ...
           updated_at = now()
         WHERE id = $1 RETURNING id, title, style, created_at, updated_at`,
        [id, hasTitle ? Array.from(title).slice(0, 200).join('') : null, style !== undefined, style ?? null]
      );
