...
import { learningView, applyLearningEdit } from './learning.js';
import * as admin from './admin.js';
import { deciderConfig, deciderProblem } from './jev.js';
import { proxyAuth, currentUser, getProfile, saveProfile, authBaseUrl, PROFILE_SWITCHES, accountInactive, deactivateAccount, isAdmin } from './auth.js';
import {
  attachmentSettings,
  createAttachment,
  getAttachmentRow,
...
async function route(req, res, pathname, user) {
  ...
  // The read-only admin dashboard (lib/admin.js). Admins only; anyone else gets 403.
  if (pathname.startsWith('/api/admin/')) {
    if (method !== 'GET') return sendJson(res, 405, { error: 'Method not allowed' });
    if (!(await isAdmin(user.id))) return sendJson(res, 403, { error: 'admin_only' });
    const q = apiQuery(req.url);
    const group = q.get('group') || 'all';
    const parts = pathname.slice('/api/admin/'.length).split('/');
    const handlers = {
      overview: () => admin.overview(group),
      students: () => (parts[1] ? admin.student(parts[1]) : admin.students(group)),
      conversations: () => (parts[1] ? admin.conversation(parts[1]) : admin.conversations(group)),
      activity: () => admin.activity(group, q.get('range')),
      misconceptions: () => admin.misconceptions(group, q.get('range')),
      tutoring: () => admin.tutoring(group),
      usage: () => admin.usage(group),
      evals: () => (parts[1] ? admin.evalRun(parts[1]) : admin.evalRuns()),
    };
  ...
  if (pathname === '/api/me/learning') {
    if (method === 'GET') return sendJson(res, 200, await learningView(user.id));
    if (method === 'POST') return sendJson(res, 200, await applyLearningEdit(user.id, await readJsonBody(req)));
    return sendJson(res, 405, { error: 'Method not allowed' });
  }
