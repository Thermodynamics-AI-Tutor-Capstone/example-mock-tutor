...
export async function runAgentTurn({
  ...
  try {
    ...
    for (let round = 1; round <= totalRounds; round++) {
      ...
      const looksUp = sendTools && !result.error && (result.toolCalls || []).some((c) => !RECORD_ONLY_TOOLS.has(c.name));
      ...
      if (!sendTools || !result.toolCalls.length) return { text, error: null, state: ctx.state, toolLog: ctx.toolLog, styleSwitch: ctx.styleSwitch };
