...
import { EXTRA_TOOLS, missingEnvFor, loadTutoringState, saveTutoringState, tutoringStateSection } from './tools.js';
...
export function styleProblem(style, hasKey) {
  ...
  if (style.tools !== 'all') {
    for (const t of style.tools) {
      if (BUILTIN_TOOL_NAMES.includes(t)) continue;
      if (!EXTRA_TOOLS[t]) return `unknown tool "${t}"`;
...
function extraToolDefs(style) {
  if (!style || style.tools === 'all') return [];
  return style.tools
    .filter((name) => EXTRA_TOOLS[name])
    .map((name) => ({ type: 'function', function: { name, description: EXTRA_TOOLS[name].description, parameters: EXTRA_TOOLS[name].parameters } }));
}
...
function statusFor(name, args) {
  switch (name) {
    ...
    default:
      if (EXTRA_TOOLS[name]?.status) return shorten(EXTRA_TOOLS[name].status(args), STATUS_TEXT_MAX);
      return `Running ${shorten(name || 'tool')}`;
  }
}

async function runTool(name, args, { skills, knowledge, allowed, ctx }) {
  if (!allowed.has(name)) return { error: `Unknown tool "${name}".` };
  try {
    if (EXTRA_TOOLS[name]) return await EXTRA_TOOLS[name].run(args, ctx);
    switch (name) {
      case 'list_course_files':
...
export async function runAgentTurn({
  ...
  let tools = [...buildTools({ skills, knowledge }).filter((t) => allowedByStyle(style, t.function.name)), ...extraToolDefs(style)];
  ...
  try {
    ...
    for (let round = 1; round <= totalRounds; round++) {
      ...
      for (const [i, call] of assistant.tool_calls.entries()) {
        ...
        const output = error ? { error } : await runTool(call.function.name, args, { skills, knowledge, allowed, ctx });
