...
export async function runAgentTurn({
  ...
  try {
    ...
    for (let round = 1; round <= totalRounds; round++) {
      ...
      if (result.error) return { text, error: result.error, state: ctx.state, toolLog: ctx.toolLog };

      if (!sendTools || !result.toolCalls.length) return { text, error: null, state: ctx.state, toolLog: ctx.toolLog };
    ...
    return { text, error: null, state: ctx.state, toolLog: ctx.toolLog };
  } catch (e) {
    return { text, error: null, exception: e, state: ctx.state, toolLog: ctx.toolLog };
  }
}
